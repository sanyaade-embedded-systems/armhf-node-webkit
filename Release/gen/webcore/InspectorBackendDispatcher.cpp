...
void InspectorBackendDispatcherImpl::Memory_getDOMNodeCount(long callId, InspectorObject*)
{
    ...
    RefPtr<TypeBuilder::Array<TypeBuilder::Memory::DOMGroup> > out_domGroups;
    RefPtr<TypeBuilder::Memory::StringStatistics> out_strings;
    ...
    if (!protocolErrors->length()) {
        ...
        if (!error.length()) {
            result->setValue("domGroups", out_domGroups);
            result->setValue("strings", out_strings);
        }
    }
    sendResponse(callId, result, commandNames[kMemory_getDOMNodeCountCmd], protocolErrors, error);
}
...
void InspectorBackendDispatcherImpl::Memory_getProcessMemoryDistribution(long callId, InspectorObject* requestMessageObject)
{
    ...
    RefPtr<InspectorObject> out_graph;
    ...
    ErrorString error;
    if (!protocolErrors->length()) {
        m_memoryAgent->getProcessMemoryDistribution(&error, reportGraph_valueFound ? &in_reportGraph : 0, out_distribution, out_graph);

        if (!error.length()) {
            result->setValue("distribution", out_distribution);
            if (out_graph)
                result->setValue("graph", out_graph);
        }
    }
...
void InspectorBackendDispatcherImpl::Timeline_setIncludeMemoryDetails(long callId, InspectorObject* requestMessageObject)
{
    ...
    sendResponse(callId, result, commandNames[kTimeline_setIncludeMemoryDetailsCmd], protocolErrors, error);
}
...
void InspectorBackendDispatcherImpl::Profiler_getProfile(long callId, InspectorObject* requestMessageObject)
{
    ...
    RefPtr<TypeBuilder::Profiler::Profile> out_profile;
    ...
    sendResponse(callId, result, commandNames[kProfiler_getProfileCmd], protocolErrors, error);
}
...
void InspectorBackendDispatcherImpl::Canvas_captureFrame(long callId, InspectorObject*)
{
    RefPtr<InspectorArray> protocolErrors = InspectorArray::create();
    ...
    ErrorString error;
    if (!protocolErrors->length()) {
        m_canvasAgent->captureFrame(&error, &out_traceLogId);
...
void InspectorBackendDispatcherImpl::Canvas_startCapturing(long callId, InspectorObject*)
{
    RefPtr<InspectorArray> protocolErrors = InspectorArray::create();
    ...
    ErrorString error;
    if (!protocolErrors->length()) {
        m_canvasAgent->startCapturing(&error, &out_traceLogId);
...
void InspectorBackendDispatcherImpl::Canvas_getTraceLog(long callId, InspectorObject* requestMessageObject)
{
    ...
    ErrorString error;
    if (!protocolErrors->length()) {
        m_canvasAgent->getTraceLog(&error, in_traceLogId, startOffset_valueFound ? &in_startOffset : 0, out_traceLog);
...
void InspectorBackendDispatcherImpl::Canvas_replayTraceLog(long callId, InspectorObject* requestMessageObject)
{
    ...
    ErrorString error;
    if (!protocolErrors->length()) {
        m_canvasAgent->replayTraceLog(&error, in_traceLogId, in_stepNo, &out_screenshotDataUrl);

        if (!error.length()) {
            result->setString("screenshotDataUrl", out_screenshotDataUrl);
        }
    }
